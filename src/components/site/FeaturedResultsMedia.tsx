    ...
    void (async () => {
      ...
      await Promise.all(
        rows.map(async (row) => {
          ...
          if (result?.signedUrl) signed[row.id] = result.signedUrl;
        }),
      );
      ...
      <div className="grid gap-x-7 gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
        {visible.map((item) => (
          <figure key={item.id} className="min-w-0 text-center">
            <div className="overflow-hidden rounded-2xl border border-gold/20 bg-[#f7f3ed] p-2 shadow-[0_12px_35px_rgba(72,52,32,0.07)]">
