...
export const describeMedia = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      data,
      context,
    }): Promise<{ ok: true; result: Described } | { ok: false; error: string }> => {
      ...
      // OpenAI receives a representative JPEG frame extracted in the browser.
      // Lovable can still receive the original video when configured.
      if (data.kind === "video" && !lovableApiKey && !data.previewDataUrl) {
        return {
          ok: false,
          error: "Could not prepare a video preview. Add the details manually.",
        };
      }

      let bytes = new Uint8Array();
      let mime = data.kind === "video" ? "video/mp4" : "image/jpeg";
      ...
      if (data.previewDataUrl) {
        content[0] = {
          type: "text",
          text: `${instruction} You are viewing a representative still frame extracted from an uploaded video.`,
        };
        content.push({
          type: "image_url",
          image_url: { url: data.previewDataUrl },
        });
